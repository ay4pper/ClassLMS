...
namespace Masteriyo\StarterTemplates\Importers\WXRImporter;
...
class WXRImporter extends WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'learning-management-system' ) );
		}
	...
	protected function log_error( WP_Error $error ) {
		...
		if ( ! empty( $data ) ) {
			masteriyo_get_logger()->debug( var_export( $data, true ) );
		}
	}
	...
	protected function import_start( $file ) {
		...
		// if ( ! is_file( $file ) ) {
		//  return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'learning-management-system' ) );
		// }
	...
	protected function process_menu_item_meta( $post_id, $data, $meta ) {
		...
		switch ( $item_type ) {
			...
			default:
				...
				masteriyo_get_logger()->debug( 'Unknown menu item type' );
				break;
		}
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'learning-management-system' ) );
		}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'learning-management-system' ) );
		}
