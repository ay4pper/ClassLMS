...
abstract class RestTermsController extends CrudController {
				...
				array(
					...
					'args'                => array_merge(
						...
						array(
							'name' => array(
								'type'        => 'string',
								'description' => __( 'Name for the resource.', 'learning-management-system' ),
								'required'    => true,
							),
						)
					),
				...
				array(
					...
					'args'                => array(
						'force' => array(
							'default'     => false,
							'type'        => 'boolean',
							'description' => __( 'Required to be true, as resource does not support trashing.', 'learning-management-system' ),
						),
					),
				),
				'schema' => array( $this, 'get_public_item_schema' ),
			...
			array(
				array(
					...
					'args'                => array(
						'ids'   => array(
							'required'    => true,
							'description' => __( 'Term IDs.', 'learning-management-system' ),
							'type'        => 'array',
						),
						'force' => array(
							'default'     => false,
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new \WP_Error( 'masteriyo_rest_cannot_view', __( 'Sorry, you cannot list resources.', 'learning-management-system' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new \WP_Error( 'masteriyo_rest_cannot_create', __( 'Sorry, you are not allowed to create resources.', 'learning-management-system' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new \WP_Error( 'masteriyo_rest_cannot_view', __( 'Sorry, you cannot view this resource.', 'learning-management-system' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new \WP_Error( 'masteriyo_rest_cannot_edit', __( 'Sorry, you are not allowed to edit this resource.', 'learning-management-system' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new \WP_Error( 'masteriyo_rest_cannot_delete', __( 'Sorry, you are not allowed to delete this resource.', 'learning-management-system' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_items_permissions_check( $request ) {
		...
		if ( ! $taxonomy || ! taxonomy_exists( $taxonomy ) ) {
			return new \WP_Error( 'masteriyo_rest_taxonomy_invalid', __( 'Taxonomy does not exist.', 'learning-management-system' ), array( 'status' => 404 ) );
		}
	...
	public function batch_items_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new \WP_Error( 'masteriyo_rest_cannot_batch', __( 'Sorry, you are not allowed to batch manipulate this resource.', 'learning-management-system' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	protected function check_permissions( $request, $context = 'read' ) {
		...
		if ( $id ) {
			...
			if ( is_wp_error( $term ) || ! $term || $term->taxonomy !== $taxonomy ) {
				return new \WP_Error( 'masteriyo_rest_term_invalid', __( 'Resource does not exist.', 'learning-management-system' ), array( 'status' => 404 ) );
			}
	...
	public function get_collection_params() {
		...
		$params['exclude'] = array(
			'description'       => __( 'Ensure result set excludes specific IDs.', 'learning-management-system' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'integer',
			),
		...
		if ( ! $taxonomy->hierarchical ) {
			$params['page'] = array(
				'description'       => __( 'Page of the items.', 'learning-management-system' ),
				'type'              => 'integer',
		...
		$params['orderby']    = array(
			'description'       => __( 'Sort collection by resource attribute.', 'learning-management-system' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_key',
			'default'           => 'id',
			'enum'              => array(
		...
		$params['hide_empty'] = array(
			'description'       => __( 'Whether to hide resources not assigned to any courses.', 'learning-management-system' ),
			'type'              => 'boolean',
			'default'           => false,
			'validate_callback' => 'rest_validate_request_arg',
		);
		if ( $taxonomy->hierarchical ) {
			$params['parent'] = array(
				'description'       => __( 'Limit result set to resources assigned to a specific parent.', 'learning-management-system' ),
				'type'              => 'integer',
				'sanitize_callback' => 'absint',
				'validate_callback' => 'rest_validate_request_arg',
			);
		...
		$params['course'] = array(
			'description'       => __( 'Limit result set to resources assigned to a specific course.', 'learning-management-system' ),
			'type'              => 'integer',
			'default'           => null,
			'validate_callback' => 'masteriyo_is_course',
		);
		$params['slug']   = array(
			'description'       => __( 'Limit result set to resources with a specific slug.', 'learning-management-system' ),
			'type'              => 'string',
			'validate_callback' => 'rest_validate_request_arg',
		);
