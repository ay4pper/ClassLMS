...
class QuizReviewsController extends CommentsController {
	...
	public function get_collection_params() {
		...
		$params['quiz'] = array(
			'default'     => array(),
			'description' => __( 'Limit result set to quiz reviews assigned to specific quiz IDs.', 'learning-management-system' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'name'         => array(
					'description' => __( 'Quiz Reviewer Author.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'email'        => array(
					'description' => __( 'Quiz Reviewer Author Email.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'url'          => array(
					'description' => __( 'Quiz Reviewer Author URL.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'ip_address'   => array(
				...
				'description'  => array(
					'description' => __( 'Quiz Review Description.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'title'        => array(
					'description' => __( 'Quiz Review Title.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'content'      => array(
					'description' => __( 'Quiz Review Content.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'required'    => true,
				),
				'rating'       => array(
					'description' => __( 'Quiz Review rating.', 'learning-management-system' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'status'       => array(
					'description' => __( 'Quiz Review Status.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'agent'        => array(
					'description' => __( 'Quiz Review Agent.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'type'         => array(
					'description' => __( 'Quiz Review Type.', 'learning-management-system' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'parent'       => array(
					'description' => __( 'Quiz Review Parent.', 'learning-management-system' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'author_id'    => array(
	...
	protected function prepare_object_for_database( $request, $creating = false ) {
		...
		if ( isset( $request['title'] ) ) {
			$quiz_review->set_title( sanitize_text_field( $request['title'] ) );
		}

		// Quiz Review Content.
		if ( isset( $request['content'] ) ) {
			$quiz_review->set_content( sanitize_text_field( $request['content'] ) );
		}

		// Quiz Review Rating.
		if ( isset( $request['rating'] ) ) {
			$quiz_review->set_rating( absint( $request['rating'] ) );
		}

		// Quiz Review Approved.
		if ( isset( $request['status'] ) ) {
			$quiz_review->set_status( sanitize_text_field( $request['status'] ) );
		}
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! $this->permission->rest_check_quiz_reviews_permissions( 'create' ) ) {
			return new \WP_Error(
				...
				__( 'Sorry, you are not allowed to create quiz reviews.', 'learning-management-system' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( isset( $request['author_id'] ) && absint( $request['author_id'] ) !== get_current_user_id() ) {
			return new \WP_Error(
				'masteriyo_rest_cannot_create',
				__( 'Sorry, you are not allowed to create quiz reviews for others.', 'learning-management-system' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( $quiz->get_author_id() === get_current_user_id() ) {
			return new \WP_Error(
				'masteriyo_rest_cannot_create',
				__( 'Sorry, you cannot create review for your own quiz.', 'learning-management-system' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( isset( $request['quiz_id'] ) ) {
			return new \WP_Error(
				'masteriyo_rest_cannot_update',
				__( 'Sorry, you cannot move a review to another quiz.', 'learning-management-system' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
