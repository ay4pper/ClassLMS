...
class LogHandlerFile extends LogHandler {
	 ...
	 *         originally used in deprecated UR_Logger::add calls.
	 * }
	 *
	...
	protected function open( $handle, $mode = 'a' ) {
		...
		if ( $file ) {
			...
			if ( $resource = @fopen( $file, $mode ) ) { // @codingStandardsIgnoreLine.
	...
	public function clear( $handle ) {
		...
		do_action( 'masteriyo_log_clear', $handle );
	...
	public function remove( $handle ) {
		...
		if ( isset( $logs[ $handle ] ) && $logs[ $handle ] ) {
			$file = realpath( trailingslashit( MASTERIYO_LOG_DIR ) . $logs[ $handle ] );
			if ( 0 === stripos( $file, realpath( trailingslashit( MASTERIYO_LOG_DIR ) ) ) && is_file( $file ) && is_writable( $file ) ) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable
				$this->close( $file ); // Close first to be certain no processes keep it alive after it is unlinked.
				$removed = unlink( $file ); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_unlink
			}
			do_action( 'masteriyo_log_remove', $handle, $removed );
		}
		return $removed;
	}
	...
	public function remove_all() {
		...
		if ( count( $logs ) ) {
			foreach ( $logs as $key => $log ) {
				$file = realpath( trailingslashit( MASTERIYO_LOG_DIR ) . $log );
				if ( 0 === stripos( $file, realpath( trailingslashit( MASTERIYO_LOG_DIR ) ) ) && is_file( $file ) && is_writable( $file ) ) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable
					$this->close( $file ); // Close first to be certain no processes keep it alive after it is unlinked.
					$removed = unlink( $file ); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_unlink
				}
	...
	public static function get_log_files() {
		$files  = @scandir( MASTERIYO_LOG_DIR ); // @codingStandardsIgnoreLine.
		$result = array();
	...
	public static function get_log_file_path( $handle ) {
		if ( function_exists( 'wp_hash' ) ) {
			return trailingslashit( MASTERIYO_LOG_DIR ) . sanitize_file_name( $handle . '-' . wp_hash( $handle ) . '.log' );
		} else {
			masteriyo_doing_it_wrong( __METHOD__, __( 'This method should not be called before plugins_loaded.', 'learning-management-system' ), '3.0' );
